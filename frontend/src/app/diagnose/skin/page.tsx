...
export default function SkinDiagnosis() {
  ...
  const handleImageSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setClassName(null); // Reset class name when new image is selected
    }
  };
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8001/segmentation", {
        method: "POST",
        body: formData,
      });
      ...
      if (data.success) {
        setSegmentedImagePath(data.segmentation.result_url);
        setClassName(data.classification.class_name);
      } else {
