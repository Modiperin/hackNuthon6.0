...
def get_yolo_models():
    detection_model=YOLO('F:\Msp_Cup\models\detection_model.pt')
    return detection_model

# Global variables to store the models
detection_model = get_yolo_models()
...
def detection_pipeline(file: UploadFile):
    print("Here")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename=f"{timestamp}_tils"
    print(filename)
    try:
        response = cloudinary.uploader.upload(file.file, folder="uploaded_images")
        return response
...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        # cloudinary_response = detection_pipeline(file)
        # print(cloudinary_response.get("url"))
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename=f"{timestamp}_tils.jpeg"
        results = detection_model(image)
        result = results[0]
        result.save(filename,labels=False)
        result_upload = cloudinary.uploader.upload('F:\Msp_Cup\\backend\\'+filename, public_id=filename)
        print(result_upload)
